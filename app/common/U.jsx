...
var U = (function () {
    ...
    var str = (function () {
        ...
        let formatIndentsId = (id) => {
            if (id) {
                return id.substr(0, 7) + '****' + id.substr(29, id.length);
            }
            return id;
        }
        ...
        return {
            isEmpty, isNotEmpty, emptyToNull, nullToEmpty, rn2br,formatMobile,formatIndentsId,
            startsWith, endsWith, replaceAll, trim, isNull, isNotNull, isIdentity,
            num2str, isChinaMobile,isValCode, trimChinaMobile, randomString, formatBankNo, bankNoTail, formatMoney, isCanvasBlank
        };
    })();
    ...
    var number_format = function (number, decimals=0, dec_point='.', thousands_sep=',') {
        ...
        * */
        ...
        var smallN = !isFinite(+number) ? 0 : +number,
            prec = !isFinite(+decimals) ? 0 : Math.abs(decimals),
            sep = (typeof thousands_sep === 'undefined') ? ',' : thousands_sep,
            dec = (typeof dec_point === 'undefined') ? '.' : dec_point,
            s = '',
            toFixedFix = function (smallN, prec) {
                var k = Math.pow(10, prec);
                return '' + Math.ceil(smallN * k) / k;
            };
     
        s = (prec ? toFixedFix(smallN, prec) : '' + Math.round(smallN)).split('.');
        var re = /(-?\d+)(\d{3})/;
        while (re.test(s[0])) {
    ...
    let num = (() => {
        ...
        let formatPrice = (price = 0, withoutPrefix) => {
            if (isNaN(price)) {
                price = 0;
            }
        ...
        return {
            num2Chinese,convertBigDecimal,formatPrice
        };
    ...
    return {
        ...
        date,
        array,
        getParameter: getParameter,number_format,toPercent,
