...
import LocationPicker from "../component/address/LocationPicker";
...
let Utils = (function () {
    ...
    let common = (() => {
        ...
        let renewStatus= (status) => {

            let ss=CTYPE.indentStatus.find(item=>item.type==status)||{label:'状态错误'};
             let { label = '', color = '' } = ss;
            return <div className='status'style={{color}} >{label}</div>
            // <Tag color={color} >{label}</Tag>
        }
    

        return {
            renewStatus,renderReactDOM, closeModalContainer, createModalContainer, scrollTop, showImgLightbox,locationPicker
        };
    })();
